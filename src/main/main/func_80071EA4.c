#include "ultra64.h"
s32 memcmp(void *, void *, s32);                  /* extern */
s32 handlerPostCmd(s32, s32, s32, s32, s32, s32, s32);     /* extern */
void entityClearSlots(s32, s32);                       /* extern */
s32 contPakFetchNote(void *, s32, void *);              /* extern */
s32 contPakCountNotes(void *, s32*);                  /* extern */
extern u16 gContPakNoteCode;
extern s32 gGameID;
extern s32 gContPakSaveData;
extern Unk gHandlerCtx;
extern s32 gContPakResult;

void contPakScanNotes(s32 arg0, s32 arg1) {
    Unk *var_s1;
    s32 temp_v0;
    s32 temp_v0_2;
    s32 var_a0;
    s32 var_s2;
    s32 var_s6;
    s8 var_s2_2;
    Unk *temp_s0;
    Unk *temp_s0_2;
    char *temp_s3;
    Unk *temp_s7;
    Unk *var_v0;

    gContPakResult = 0;
loop_1:
loop_2:
    gHandlerCtx.unk168 = 1;
    var_s2 = 0xF;
    var_v0 = &gHandlerCtx + 0x294;
    do {
        var_v0->unk38C = 0;
        var_s2 -= 1;
        var_v0 -= 0x2C;
    } while (var_s2 >= 0);
    temp_s0 = (arg1 * 4) + &gHandlerCtx;
    if ((temp_s0->unk31C != 2) || (temp_s0->unk34C != 0)) {
        entityClearSlots(arg1, 2);
        if (temp_s0->unk31C != 2) {
            gHandlerCtx.unk168 = 0;
        }
    }
    if (gHandlerCtx.unk168 != 0) {
        temp_v0 = contPakCountNotes((arg1 * 0x68) + (&gHandlerCtx + 0x17C), &gHandlerCtx + 0x64C);
        if (temp_v0 != 0) {
            if (temp_v0 == 2) {
                temp_s0_2 = (arg1 * 4) + &gHandlerCtx;
                temp_s0_2->unk31C = 6;
                entityClearSlots(arg1, 2);
                if (temp_s0_2->unk31C != temp_v0) {
                    gHandlerCtx.unk168 = 0;
                } else {
                    goto loop_2;
                }
            } else if (handlerPostCmd(0x3C, arg1, 0x52, 0x31, 0, 0, 0) != 0) {
                ((arg1 * 4) + &gHandlerCtx)->unk31C = 3;
                gHandlerCtx.unk168 = 0;
            } else {
                goto loop_1;
            }
        }
    }
    var_s2_2 = 0;
    temp_s7 = (arg1 * 4) + &gHandlerCtx;
    var_s1 = &gHandlerCtx;
    var_s6 = 0;
loop_17:
    if (gHandlerCtx.unk168 != 0) {
        temp_s3 = var_s6 + (&gHandlerCtx + 0x398);
        temp_v0_2 = contPakFetchNote((arg1 * 0x68) + (&gHandlerCtx + 0x17C), var_s2_2, temp_s3);
        switch (temp_v0_2) {                        /* irregular */
        case 0:
            var_a0 = 0;
            if (var_s1->unk3A0 == gContPakNoteCode) {
                if ((var_s1->unk39C == gGameID) && (var_s1->unk3A2 == 0)) {
                    var_a0 = memcmp(temp_s3 + 0xE, &gContPakSaveData, 0x10) == 0;
                }
            }
            var_s1->unk394 = var_s2_2;
            var_s1->unk390 = var_a0;
            var_s1->unk38C = 1;
            var_s1 += 0x2C;
            var_s6 += 0x2C;
        case 5:
block_35:
            var_s2_2 += 1;
            if (var_s2_2 < 0x10) {
                goto loop_17;
            }
            break;
        case 2:
            temp_s7->unk31C = 6;
            entityClearSlots(arg1, 2);
            if (temp_s7->unk31C != temp_v0_2) {
                gHandlerCtx.unk168 = 0;
                goto block_35;
            }
            goto loop_2;
        default:
            if (handlerPostCmd(0x3C, arg1, 0x52, 0x31, 0, 0, 0) != 0) {
                temp_s7->unk31C = 3;
                gHandlerCtx.unk168 = 0;
                goto block_35;
            }
            goto loop_2;
        }
    }
    gHandlerCtx.unk160 = (s32) (gHandlerCtx.unk160 - 1);
}

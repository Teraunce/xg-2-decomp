...
def parse_expected_size(stub_path: str, input_obj: str = None) -> int:
    ...
    for line in lines:
        ...
        if m:
            functions.append((m.group(1), int(m.group(2), 0)))

    if not functions:
        raise ValueError(f"No 'nonmatching' size found in {stub_path}")

    # Determine which secondary symbols are compiled into the object
    defined = get_defined_symbols(input_obj) if input_obj else set()

    total = functions[0][1]  # Always include primary
    for fname, size in functions[1:]:
        if fname in defined:
            total += size
